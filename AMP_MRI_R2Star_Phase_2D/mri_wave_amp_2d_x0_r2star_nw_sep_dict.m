    ...
    sampling_rate = 0.1;        % make sure to do full sampling in the AC region
    ...
    output_file = '../result/gamp_rec_2d';     % the prefix for the 2d output 
    ...
    x_mag_max = 1;      % maximum value of magnitude image
    x_mag_min = eps;    % minimum value of magnitude image
    x0_max = 1;         % maximum value of the initial magnetization
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    load('../data/sensitivity_map_2d.mat')  % get maps
    ...
    for (i=1:Ne)
        x_hat_meas_psi(:,i) = Psi(x_hat_meas(:,:,i));
    end
    ...
    output_par.tau_w_2 = 1e-12;

    % construct measurement operator
    hS = @(in1, in2) A_op(in1, in2);
    hSt = @(in1, in2) At_op(in1, in2, mat_sz);

    M=num_samples*Ne*Nc;
    N=wav_coef_len*Ne;
    A_2_echo = A_2_echo_LinTrans(M,N,maps,mat_sz,num_samples,Ne,Nc,sampling_vect,Psit,Psi,hS,hSt);

    M=num_samples*Ne*Nc;
    N=sy*sz*Ne;
    A_2_echo_nw = A_2_echo_nw_LinTrans(M,N,maps,mat_sz,num_samples,Ne,Nc,sampling_vect,hS,hSt);

    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %% multi-echo reconstruction %%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    [res, input_par_new, output_par_new] = gamp_mri_multi_echo_2d_nw(A_2_echo, y_echo, gamp_par, input_par, output_par);
    ...
    M=sy*sz*Ne;
    N=wav_coef_len;
    A_x0_r2star = A_x0_r2star_LinTrans(M,N,mat_sz,wav_coef_len,Ne,echo_time,Psit,Psi);

    M=sy*sz*Ne;
    N=wav_coef_len*Ne;
    A_wav = A_wav_LinTrans(M,N,mat_sz,wav_coef_len,Ne,Psit,Psi);

    M=sy*sz;
    N=wav_coef_len;
    A_wav_single = A_wav_single_LinTrans(M,N,mat_sz,wav_coef_len,Psit,Psi);
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%
    [res, input_par_new, output_par_new] = gamp_mri_x0_r2star_2d_nw_dict(A_2_echo_nw, A_wav, A_wav_single, A_x0_r2star, y_echo, gamp_par, input_par, output_par);
    ...
    x0_rec = ifftshift(Psit(res.x0_psi));   % recovered initial magnetization
    ...
    figure; imshow(x0_rec,[0,0.015])
    figure; imshow(r2star_rec,[0,0.05])
    figure; imshow(real(x_hat_rec(:,:,1)),[-0.015,0.015])
