classdef A_2_echo_nw_3d_cylinder_LinTrans < LinTrans
	...
	methods
		...
		% Constructor
		function obj = A_2_echo_nw_3d_cylinder_LinTrans(M,N,maps,mat_sz,sm_num,echo_num,coil_num,s_vect,A,Ah,S,St)
			obj = obj@LinTrans;
			...
			if nargin > 10
				...
				for p=1:P,   % use "for" since A may not support matrices 
					...
					x_tmp = randn([mat_sz(1) mat_sz(2) mat_sz(3) echo_num]) + 1i * randn([mat_sz(1) mat_sz(2) mat_sz(3) echo_num]);
			...
			if (M~=mat_sz(1)*sm_num*echo_num*coil_num)
				error('Dimension mismatch: M')
			end
			
			if (N~=mat_sz(1)*mat_sz(2)*mat_sz(3)*echo_num)
		...
		function y = mult(obj,x)
			y=zeros([obj.mat_sz(1) obj.sm_num obj.echo_num obj.coil_num]);
		...
		function x = multTr(obj,y)
			...
			for (j=1:obj.echo_num)
				for (k=1:obj.coil_num)
					x_tmp(:,:,:,j) = x_tmp(:,:,:,j) + obj.mat_sz(1)*obj.mat_sz(2)*obj.mat_sz(3)*obj.maps_conj(:,:,:,k).*obj.Ah(y(:,:,j,k), obj.s_vect(:,j));
				end
			end
            x = x_tmp;
		end
		...
		function y = multSq(obj,x)
			if isempty(obj.FrobNorm)
				...
				%y = obj.echo_num*ones([obj.mat_sz(1) obj.sm_num obj.echo_num obj.coil_num])*((obj.FrobNorm^2/(obj.M*obj.N))*sum(x,'all'));
		...
		function x = multSqTr(obj,y)
			if isempty(obj.FrobNorm)
				...
				%x = obj.echo_num*ones([obj.mat_sz(1) obj.mat_sz(2) obj.mat_sz(3) obj.echo_num])*((obj.FrobNorm^2/(obj.M*obj.N))*sum(y,'all'));
