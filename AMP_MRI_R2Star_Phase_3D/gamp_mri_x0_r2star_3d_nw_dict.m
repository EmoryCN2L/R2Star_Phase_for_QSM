function [res, input_par, output_par] = gamp_mri_x0_r2star_3d_nw_dict(A_2_echo_nw_3d, A_wav_3d, A_wav_single_3d, A_x0_r2star_3d, y, gamp_par, input_par, output_par)
    ...
    tau_x_mag_meas_1 = A_wav_3d.multSq(tau_x_meas_psi);
    x_mag_meas_1_complex = A_wav_3d.mult(x_hat_meas_psi);
    ...
    H_exp_psi = A_wav_single_3d.multTr(H_exp);
    ...
    % initialize the A_x0_r2star_3d operator
    set(A_x0_r2star_3d, 'R2star', r2star);
    A_x0_r2star_3d.estFrob();

    %%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %% warm up the iterations %%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%
    tau_p_exp = A_x0_r2star_3d.multSq(tau_x0_psi);
    p_hat_exp = A_x0_r2star_3d.mult(x0_psi); % - tau_p_exp .* s_hat_exp;
	...
	for (ite_pe = 1:max_pe_ite)
            ...
            tau_p_meas_2 = A_2_echo_nw_3d.multSq(tau_x_hat_all);
            p_hat_meas_2 = A_2_echo_nw_3d.mult(x_hat_all) - tau_p_meas_2 * s_hat_meas_2;
            ...
            tau_r_meas_2 = 1 / A_2_echo_nw_3d.multSqTr(tau_s_meas_2);
            r_hat_meas_2 = x_hat_all + tau_r_meas_2 * A_2_echo_nw_3d.multTr(s_hat_meas_2);
            ...
            tau_r_exp = 1 / A_x0_r2star_3d.multSqTr(tau_s_exp);
            r_hat_exp = x0_psi + tau_r_exp * A_x0_r2star_3d.multTr(s_hat_exp);
            ...
            x0_tmp = A_wav_single_3d.mult(x0_psi);
            x0_tmp = min(x0_tmp, x0_max);
            x0_tmp = max(x0_tmp, x0_min);
            x0_psi = A_wav_single_3d.multTr(x0_tmp);

            cvg_x0_psi = norm(x0_psi(:)-x0_psi_pre(:), 'fro')/norm(x0_psi(:), 'fro');

            x0 = A_wav_single_3d.mult(x0_psi);
            x0 = min(x0, x0_max);
            x0 = max(x0, x0_min);

            r2star = exp_lsq_fit_3d_r2_componentwise_dict(x_mag_meas, x0, exp_dict, exp_dict_sq_norm, r2_dict_val);
