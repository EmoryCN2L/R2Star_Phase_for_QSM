function [pts] = poissonDisc_complement(sizeI,spacing,preSelect,nPts,showIter)

% modify the search with some locations removed
...
if nargin == 4; showIter = 0; end
if nargin == 3; showIter = 0; nPts = 0; end
...
if (size(preSelect,1)>0)
for (i=1:size(preSelect,1))
    dist_tmp = sqrt(sum((sGrid_ceil-preSelect(i,:)).^2, 2));
    emptyGrid(dist_tmp==0)=0;
end
end
