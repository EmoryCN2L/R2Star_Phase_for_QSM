function [Stp1, fun_val_block_1_cur_0, fun_val_block_1_cur_1] =recovery_multi_wave_l1_reg_2d(Y, maps, lambda_1, Psi, Psit, par)
    ...
    mat_sz = [size(St, 1) size(St, 2)];
    sy = size(St,1);
    sz = size(St,2);
    echo_num = size(St,3);
    coil_num = size(maps, 3);
    ...
    for (j=1:echo_num)
        for (k=1:coil_num)
            ...
            P(:,:,j,k)=Rt(:,:,j)-1/kappa*2*sy*sz*maps_conj(:,:,k).*At_op(A_op(maps(:,:,k).*Rt(:,:,j), s_vect(:,j))-Y(:,j,k), s_vect(:,j), mat_sz);
        end
    end
    ...
    for (j=1:echo_num)
        for (k=1:coil_num)
            fun_val_block_1_cur_0 = fun_val_block_1_cur_0 + norm(Y(:,j,k)-A_op(maps(:,:,k).*Stp1(:,:,j), s_vect(:,j)), 'fro')^2;
        end
    end

    fun_val_block_1_cur_1 = zeros(1,echo_num);
    for (j=1:echo_num)
        fun_val_block_1_cur_1(j) = sum(abs(Psi(Stp1(:,:,j))), 'all');
    end
