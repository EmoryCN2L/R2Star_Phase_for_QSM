 function V=exp_lsq_fit_3d_r2_componentwise_dict(Et, H_exp, exp_dict, exp_dict_sq_norm, r2_dict_val)
    ...
    Et_process_sq_norm = sum(Et_process.^2,4);
    
    min_distance_to_dict = zeros(sx,sy,sz);
    V = zeros(sx,sy,sz);
    for (i=1:size(exp_dict,2))
        ...
        if (i==1)
            V = repmat(r2_dict_val(i),[sx sy sz]);
