    ...
    lambda_1 = 0.1;     % regularization parameter for recovering multi-echo images prior
    ...
    output_file = '../result/l1_reg_rec_3d';     % the prefix for the 3d output 
    ...
    kappa = zeros(1, Nc);   % compute lipschitz constant for FISTA
    for (j=1:Nc)
        kappa(j) = 2*sx*sy*sz*(max(abs(maps_3d(:,:,:,j)), [], 'all')).^2;
    end
    kappa = max(kappa(:));
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    [X_rec_echo, fun_val_block_1_cur_0, fun_val_block_1_cur_1] = recovery_multi_wave_l1_reg_3d(y_echo, maps_3d, lambda_1, Psi, Psit, E_coef, C_struct, par);
    ...
    x0_rec = ifftshift(X0_rec);             % recovered initial magnetization
    r2star_rec = ifftshift(R2_star_rec);    % recovered R2star map
